...
const Home = () => {
  ...
  const getTransitionStyle = (isEntering) => {
    if (transitioning) {
      ...
      if (!isEntering) {
        return {
          transform: `scale(0.85) rotate(${prevRotation + transitionDirection * 5}deg)`,
          opacity: 0,
        };
      }
      // For entering image: start scaled down and rotated away, then animate to final position
      return {
        transform: `scale(0.85) rotate(${rotation - transitionDirection * 5}deg)`,
        opacity: 0,
      };
    }
    ...
    <div className="flex w-full flex-row gap-16 md:gap-24 lg:gap-32 xl:gap-48">
      ...
      <div className="flex flex-1 items-center justify-center relative">
        ...
        <div className="w-full aspect-square relative">
          {/* Current image with rotation effect */}
          <div
          ...
          {/* Previous image with rotation effect, only shown during transition */}
          {prevImage && (
            <div
              className="absolute inset-0 transition-all duration-300 ease-in-out"
